		...
		protected static function callCurl($arParams)
		{
			...
			if($arSettings !== false)
			{
				...
				try
				{
					...
					// disable Expect 100-continue headers to prevent empty server's answers --@vdb
					curl_setopt($obCurl, CURLOPT_HTTPHEADER, array('Expect:'));
					// set descent CONNECTTIMEOUT (default: 300 second) --@vdb
					curl_setopt($obCurl, CURLOPT_CONNECTTIMEOUT, 10); // 10 seconds is enough
					// stop on http errors >= 400 --@vdb
					curl_setopt($obCurl, CURLOPT_FAILONERROR, true);
					...
					if (! $result) { echo 'Сбой: '; print_r($out); }
			...
			else
			{
				...
				return [
					'error' => '$arSettings is false',
					'error_information' => 'error in $arSettings '
				];
			}
